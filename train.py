...
def train_one_epoch(train_dataloader, model, optimizer, loss):
    ...
    if torch.cuda.is_available():
        model.cuda()# YOUR CODE HERE: transfer the model to the GPU
        # HINT: use .cuda()
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    scheduler  = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.8, patience=5,threshold = .01)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if valid_loss_min is None or (
                (valid_loss_min - valid_loss) / valid_loss_min > 0.0
        ):
